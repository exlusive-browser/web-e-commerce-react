...
export const ProductDetail: React.FC<ProductDetailProps> = ({ id }) => {
  ...
  const images = [getImgUrl(data.image), getImgUrl(data.hoverImage)];

  return (
    <section
      className="product-desc2 px-6 md:px-24 bg-white rounded-md"
      id="product-desc2"
    >
      <h1 className="text-2xl md:text-3xl font-bold mb-4 text-[#333] py-6">
        {data.title}
      </h1>
      <div className="container-sec1 flex flex-col xl:flex-row gap-6">
        ...
        <div className="desc-container flex-1">
          ...
          <div className="flex items-center space-x-2">
            <p className="text-white bg-[#2e86c1] p-1 px-2 rounded text-xs">
              {data.discount}
            </p>
          ...
          <div className="flex items-center gap-4">
            <div className="quantity-selector inline-block border border-[#3498db] p-2 rounded-md">
              <select
                id="quantity"
                className="quantity-dropdown text-base bg-white p-1 cursor-pointer outline-none border-none"
              >

      ...
      {products.map((product, index) => {
        ...
        return (
          <div
            ...
            <img src={product.img} alt={product.name} className="w-full lg:w-[35%] object-cover h-64 lg:h-auto" />
            <div className="p-5 flex flex-col justify-between flex-1">
              <div>
                <h2 className="text-lg md:text-2xl font-semibold my-1">{product.name}</h2>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-3 items-start sm:items-center">
                {product.discount > 0 ? (
                  ...
                  <p className="text-lg sm:text-xl md:text-2xl font-bold">${product.price.toLocaleString()}</p>
                )}
              </div>

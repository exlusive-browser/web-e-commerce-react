...
export function ProductListCart() {
  ...
  const calculateDiscounts = () => {
    return cartProducts.map((product) => {
      const price = parseFloat(product.originalPrice.replace(/\$|\.|,/g, ''));
      const discountValue = parseFloat(product.discount.replace(/-|%/g, '')) / 100;
  ...
  const totalDiscount = discounts.reduce((total, discount) => total + discount, 0);
  const total = subtotal - totalDiscount;

  return (
    <MainLayout>
      <DynamicBreadcrumb />
      <h1 className="pl-10 pt-5 pb-5 text-3xl text-gray-800 font-bold  text-center">SHOPPING CART</h1>
      <div className="flex flex-col-reverse justify-center lg:flex-row p-[50px] lg:p-[50px] ">
        <div className="w-full lg:w-3/5 max-w-[900px] px-50">
          <ProductItemCart products={cartProducts} setCartProducts={setCartProducts} />
        </div>
        <div className="w-full lg:w-2/5 p-[50px]">
          <PurchaseOverview subtotal={subtotal} discounts={discounts} total={total} />
        </div>
      </div>
    </MainLayout>
  );
